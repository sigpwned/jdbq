...
public final class OptionalCollectors {
   ...
   * The returned collector will throw {@code IllegalStateException} whenever 2 or more elements are
   * present in a stream. Null elements are mapped to {@code Optional.empty()}.
   *
   ...
   * {@code OptionalInt}. The returned collector will throw {@code IllegalStateException} whenever 2
   * or more elements are present in a stream. Null elements are mapped to
   ...
   * optional-style container type. The returned collector will throw {@code IllegalStateException}
   * whenever 2 or more elements are present in a stream. Null elements are mapped to an empty
   * container.
   *
  ...
  public static <T, O> Collector<T, ?, O> toOptional(Supplier<O> empty, Function<T, O> factory) {
    return Collector.of(() -> new OptionalBuilder<>(empty, factory), OptionalBuilder::set,
        OptionalBuilder::combine, OptionalBuilder::build);
  }
}
