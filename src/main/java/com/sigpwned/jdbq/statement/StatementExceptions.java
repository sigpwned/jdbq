 ...
 */
@Beta
public class StatementExceptions implements JdbqConfig<StatementExceptions> {
  ...
  public enum MessageRendering implements Function<StatementException, String> {
    ...
    PARAMETERS {
      @Override
      public String render(StatementException exc, StatementContext ctx) {
        return String.format("%s [arguments:%s]", exc.getShortMessage(), ctx.getArgumentBinding());
      }
    },
    /**
    ...
    SHORT_STATEMENT {
      ...
      public String render(StatementException exc, StatementContext ctx) {
        ...
        return String.format("%s [statement:\"%s\", arguments:%s]", exc.getShortMessage(),
            limit(ctx.getRenderedSql(), limit), limit(ctx.getArgumentBinding().toString(), limit));
      }
    ...
    DETAIL {
      ...
      public String render(StatementException exc, StatementContext ctx) {
        return String.format("%s [statement:\"%s\", rewritten:\"%s\", parsed:\"%s\", arguments:%s]",
            exc.getShortMessage(), ctx.getRawSql(), ctx.getRenderedSql(), ctx.getParsedSql(),
            ctx.getArgumentBinding());
      }
    };
